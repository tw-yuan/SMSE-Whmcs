...
function smse_fami_MetaData()
{
    return array(
        'DisplayName' => 'smse_fami',
        'APIVersion' => '1.0',
...
function smse_fami_link($params)
{
    // 網關參數
    $Dcvc = $params['Dcvc'];
    ...
    if (($result->FamiNO != null)) {
        $code = '<div class="text-left alert alert-info"><p><b>繳費代碼：</b><code>' . $result->FamiNO . '</code></p>' .
            '<p><b>應繳金額：</b><code>' . $result->Amount . '</code></p>' .
            '<p><b>繳費期限：</b><code>' . $result->FamiNO_PayEndDate . '</code></p></div>';
        return $code;
    } else {
        //交易參數
        $data = array(
        ...
        Capsule::table('smse_pay_record')->updateOrInsert(
            ...
            [
                'FamiNO' => $data['FamiNO'],
                'Amount' => $data['Amount'],
                'FamiNO_PayEndDate' => $data['PayEndDate'],
                'Fami_SmilePayNO' => $data['SmilePayNO'],
            ]
        );

        $code = '<div class="text-left alert alert-info"><p><b>繳費代碼：</b><code>' . $data['FamiNO'] . '</code></p>' .
            '<p><b>應繳金額：</b><code>' . $data['Amount'] . '</code></p>' .
