...
function smse_bank_MetaData()
{
    return array(
        'DisplayName' => 'smse_bank',
        'APIVersion' => '1.0',
...
function smse_bank_link($params)
{
    // 網關參數
    $Dcvc = $params['Dcvc'];
    ...
    if (($result->AtmNo) != null) {
        $code = '<div class="text-left alert alert-info"><p><b>銀行代碼：</b><code>' . $result->AtmBankNo . '</code></p>' .
            '<p><b>繳費代碼：</b><code>' . $result->AtmNo . '</code></p>' .
            '<p><b>應繳金額：</b><code>' . $result->Amount . ' TWD</code></p>' .
            '<p><b>繳費期限：</b><code>' . $result->atm_PayEndDate . '</code></p></div>';
        return $code;
    } else {
        //交易參數
        $data = array(
        ...
        Capsule::table('smse_pay_record')->updateOrInsert(
            ...
            [
                'AtmBankNo' => $data['AtmBankNo'],
                'AtmNo' => $data['AtmNo'],
                'Amount' => $data['Amount'],
                'atm_PayEndDate' => $data['PayEndDate'],
                'atm_SmilePayNO' => $data['SmilePayNO'],
            ]
        );

        $code = '<div class="text-left alert alert-info"><p><b>銀行代碼：</b><code>' . $data['AtmBankNo'] . '</code></p>' .
            '<p><b>繳費代碼：</b><code>' . $data['AtmNo'] . '</code></p>' .
